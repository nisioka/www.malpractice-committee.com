...
4. **記事化**（新規事案のみ）— 1件ごとに次のJSONを作る（配列で複数可）:
   ...
   **ガードレール（YMYL・医療分野のため厳守）**:
   ...
   - **略称・通称だけで書かない。初出で正式名称を併記する。** `body_html` の初出を
     「体外式膜型人工肺「ECMO（エクモ）」」の形にし、以降は通称でよい。`title` にも
     ラテン文字の略称を残す。検索結果とSNSカードに出るのは `title` だけなので、本文にしか
     正式名称が無いと、通称で検索した人にしか届かない。
